---
title: מומחה אוכל ומוצר
tags:
  - project/agent
  - stage/vision
  - stage/selection
aliases:
  - food-and-product
---

# מומחה אוכל ומוצר

יודע אילו רגעים מוכרים מנה או מוצר — הרגע שבו זה נראה הכי טוב, ולא הרגע שבו מדברים עליו.

חלק מ-[[Agents]] · חלק מ-[[Premier Edit]].

## שלבים

ראייה, בחירת תוכן — ראה [[Pipeline and Agents]].

- **ראייה** — אוכל ומוצר — מה חשוב לתאר
- **בחירת תוכן** — אוכל ומוצר — מה באמת מוכר

## עובד מול

[[framing]], [[platform-feed]], [[hebrew]]

נקרא על ידי: [[platform-feed]], [[framing]], [[hebrew]], [[cinematography]]

## קוד

`lib/experts/food-and-product.ts` — ראה [[Tech Stack#Expert layer]].

## מקורות

- CLAUDE.md — MVP scope: social media content only
