...
> [!success] Sync is the free half of making sound and picture agree (2026-08-02)
> The user named the central product problem: audio content and visual content are planned separately and must be made to work together. Full semantic matching needs the model. But `isSyncFor` — same clip, same source time as the words being heard — is free, was already computed, and was being read only by the LLM prompt. Wiring it into the heuristic planner took 1/14 placements in sync to 5/14 with no loss of diversity. **The bonus (0.20) is explicitly a judgement, not a measurement** — unlike the timing constants — and is capped at two in a row, because unbroken sync is the untouched take rather than an edit.

> [!success] Extra picture after the audio ends is wanted (2026-08-02)
> Recorded because I assumed the opposite and was wrong. The tail of picture running past the end of the spine is **useful** to the user — *"זה נותן לי אופציה להשתמש עם זה אחרי זה גם."* The staleness fix stands on its own terms (a layer laid out against a different, longer spine is describing a cut that no longer exists), but "bonus material at the tail" is a legitimate feature request, not a defect to be eliminated. If it comes back it should be deliberate — chosen shots, labelled as spare — not a leftover.
...
> [!success] When a reviewer finds something a computer could have found, put it in the computer (2026-08-03)
> The `script-critic` agent read a 15-line script and noticed that seven joins were the same clip resuming where it stopped — so the ear heard **eight** moments, with a 9.48-second unbroken take inside. Every line passed validation; the fault was in the *seam*, which no per-line rule can see. But it is pure arithmetic. It is now a validator warning that reproduces the critic's read exactly. **Judgement is expensive and arithmetic is free — every time a critique turns out to be countable, it should stop being a critique.**

> [!success] Agents declare their craft choices, not just their output (2026-08-03)
> `script-writer` names its hook type, retention device and what the ending is for, inside `beats`. This started as the hook expert's idea and generalised. Two payoffs: it stops the writer reaching for whatever felt good and rationalising it afterwards, and it gives the critic something falsifiable to check. *A declared open loop that never closes is a finding; "the middle drags" is an opinion.*
...
> [!warning] Assert that an automated edit changed the text (2026-08-02)
> Two find-and-replace patches in one session matched nothing and reported nothing. The suite then passed — not because the code was right, but because the test that would have caught it was never added, and the fixture it needed was never wired. **A no-op patch is indistinguishable from a successful one unless you check.** `git diff` after editing, or assert the string actually changed. This is the same family as the entry below and now sits in the `red-first` agent: the third false green of the day, and the only one that was self-inflicted.
...
- **Semantic matching between words and picture.** Sync (above) is the free half and covers the case where the speaker was filmed doing the thing they are describing. It does nothing for the common case where the words name something the camera covered *at a different time*: `npm run coherence` shows the cut saying *"יש פה מרווה, זאתר, אלוויזה וזוטה לבנה"* over olive saplings, a kettle and a campfire, with the named herbs nowhere on screen — even though the vision descriptions for those shots exist in the database. Matching them needs either the model, or a free keyword match between the transcript text and the stored `VisualAnalysis` descriptions. **The free version has not been tried and might get surprisingly far**, since the descriptions are already Hebrew text about what is in frame.

- **A selected moment with no transcript text under it.** `npm run coherence` shows a 5-second moment in *חליטת תה - סט מוקטן* with no words at all. Either the spine selected something silent, or the moment's times do not line up with the transcript segments it came from. Not investigated.

- **The written spine draws on two clips of nine.** Raised by the writer itself, unprompted: the best writing in this corpus lives in two clips, so the audio spine concentrates there and **source diversity has to come entirely from the picture layer**. That is a fair trade for a stronger story, but it makes Stage B load-bearing rather than decorative — and it may not hold on footage where the good lines are spread more evenly. Worth re-checking on the next project.
