...
> [!success] RTL → whole document flipped, both surfaces (2026-07-31)
> The product had only scattered per-element `dir="auto"` patches, never a real document direction. Decided to flip the whole document to `dir="rtl" lang="he"` (web app + UXP panel) rather than continue patching — the standard approach for a Hebrew-primary product. The non-obvious part: flexbox layouts already mirror correctly with zero code changes (direction-aware by the CSS spec), but plain-English text combining digits/punctuation/multiple word-groups bidi-garbles under RTL with no explicit override — confirmed by screenshot, not assumed. Fixed with explicit `dir="ltr"` islands on English-chrome text and `dir="auto"` on Hebrew content, verified via computed-style checks. See `docs/superpowers/specs/2026-07-31-rtl-support-design.md`.
...
- **Does Adobe Spectrum (`sp-*`) actually honor `dir="rtl"` inside real Premiere?** Stated from general knowledge that Spectrum supports RTL locales (Creative Cloud ships in Hebrew/Arabic) — not confirmed against this project's actual UXP runtime, same limitation as everything panel-related. Check this alongside the `sp-picker` `change` event below next time the panel is reloaded.
