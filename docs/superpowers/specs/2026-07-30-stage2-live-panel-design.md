...
This is new, separate work from the in-flight `editing-quality` branch (word-boundary cuts, audio crossfades, editorial rubric). Original recommendation was to land `editing-quality` first since its output (`CutClip[]` with `audioInSec`/`audioOutSec`) is exactly what this panel's execution phase will consume.

**Revised 2026-07-30, same day:** the user asked to prioritize getting *something* loadable into Premiere immediately, rather than waiting for `editing-quality` to fully close out. Splitting v1 into two phases resolves this without abandoning the original sequencing logic — only the phase that actually needs `CutClip[]`'s final shape stays gated on `editing-quality`:

- **Phase 1 (read-only walking skeleton) — does not depend on `CutClip[]` at all.** Built directly, no plan/SDD ceremony (small, well-grounded against Adobe's own `AdobeDocs/uxp-premiere-pro-samples` repo, not guessed): `premiere-panel/manifest.json` + `index.html` + `index.js` + `styles.css` + `README.md`. Reads the active project, active sequence, and video-track-1 clip list via the real `premierepro` UXP API (`ppro.Project.getActiveProject()`, `project.getActiveSequence()`, `sequence.getVideoTrack(0)`, `track.getTrackItems(...)`) and displays them in the panel. No writes, no execution, nothing destructive — this is purely a connectivity proof so the user has something to load via UXP Developer Tool and see react to their actual project.
- **Phase 2 (plan execution)** — everything described in "Approach" above (`CutClip[]` → `ppro.Sequence` insert calls, audio-crossfade transitions, the one confirmation step). Still gated on `editing-quality` merging, since it consumes that branch's final `CutClip[]` shape.

One path deviation from the original doc: the panel lives at repo-root `premiere-panel/`, not `lib/premiere-panel/` as first proposed. `lib/` is the Next.js app's server-side module boundary (imported via `@/lib/...`); a UXP plugin has its own `manifest.json`/runtime and isn't Next.js code, so nesting it inside `lib/` would invite Next's build/typecheck to treat it as app code. Root-level, sibling to `scripts/` (the other non-Next.js runtime in this repo), matches existing precedent.

Phase 1 status: implemented on branch `stage2-panel`, **not yet loaded into a real Premiere install** — that verification is the user's next step, same "load via UXP Developer Tool" flow described in `premiere-panel/README.md`.

Roadmap order after this spec: **(1)** Stage 2 Phase 1 (walking skeleton, done ahead of schedule per above), **(2)** finish `editing-quality`, **(3)** Stage 2 Phase 2 (plan execution) once `editing-quality` merges, **(4)** filler-word/silence detection, **(5)** micro-cut merging + bounds validation.
